...
const healthyPet = document.getElementById('healthy-btn');// const buttonCaculateBMI = document.getElementById('caculate-bmi');
const healthyPetList = [];
// let listPetBMI = [];
let checkHealthyPet = false;
let checkResultBMI = false;
let checkUpdateInfo = false;
let breedList = getFromStorage(KEYBREED) ?? [];
...
//reload data breed from storage when it changes 
function reloadDataBreedFromStoreage() {
    window.addEventListener('storage', function (e) {
        if (e.key === KEYBREED) {
            breedList = getFromStorage(KEYBREED);
            console.log(breedList);
        }
    })
}
reloadDataBreedFromStoreage();
